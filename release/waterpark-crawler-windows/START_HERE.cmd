...
chcp 65001 >nul
cd /d "%~dp0"
title Welli Waterpark Sales Crawler
color 0B
...
if not defined NODE_EXE (
  ...
  echo.
  echo 해결 방법:
  echo  1. https://nodejs.org/ko 에 접속
  echo  2. LTS 버전 설치
  echo  3. 설치 후 이 창을 닫고 START_HERE.cmd 를 다시 실행
  echo.
  echo 설치 페이지를 여는 중입니다...
...
if not exist "node_modules" (
  ...
  echo 인터넷 상태에 따라 1분 정도 걸릴 수 있습니다.
  echo.
...
echo.
echo [실행 시작] 15분마다 최근 10일 워터파크 매출을 자동 수집합니다.
echo 이 창을 닫으면 자동 수집이 멈춥니다.
echo.

"%NODE_EXE%" waterpark-auto-crawler.cjs watch --days 10 --interval 15
