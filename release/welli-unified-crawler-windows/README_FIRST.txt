...
4. 워터파크 일일 실시간 매출
   - 홈페이지의 "최신 매출 동기화" 버튼을 누르면 Cloudflare 서버가 직접 수집합니다.
   - 이 윈도우 PC에서는 워터파크 매출을 수집하지 않습니다.
...
- 기본값: 15분마다 관리자 데이터(VOC/시즌권/패키지)를 다시 확인합니다.
- 중복 저장이 아니라 같은 주문번호/날짜는 Supabase에서 업데이트됩니다.
- 웹 대시보드의 "최신 시즌권 동기화" 요청을 4초마다 확인해 즉시 실행합니다.
