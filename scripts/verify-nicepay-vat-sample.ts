...
import { processVatSettlement, type RawRow } from "../src/components/nicepayVatEngine.ts";
...
const rows: RawRow[] = matrix.slice(1).filter((row) => row.some((value) => value !== "" && value !== null)).map((row) => Object.fromEntries(headers.map((header, index) => [header, row[index] ?? ""])));
