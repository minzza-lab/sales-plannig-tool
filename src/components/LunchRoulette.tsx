...
const LunchRoulette = () => {
  ...
  useEffect(() => {
    ...
    const handleGoal = (event: Event) => {
      ...
      setIsRacing(false);
      setNotice(`${winners.join(', ')}님${winners.length > 1 ? '들이' : '이'} ${winnerMode === 'last' ? '마지막까지 살아남은 오늘의 꼴찌입니다!' : '오늘의 음료 담당입니다!'}`);
    };
  ...
  const startRace = () => {
    ...
    roulette.setWinnerRange(winnerMode === 'last' ? totalBalls - 1 : 0, winnerMode === 'last' ? totalBalls - 1 : Math.min(winnerCount, participants.length) - 1);
    roulette.start();
    setResults([]); setIsRacing(true); setNotice(winnerMode === 'last' ? '꼴찌 추첨을 시작했습니다. 마지막 남은 마블을 카메라가 하이라이트합니다.' : '마블 레이스가 시작됐습니다. 원본 엔진의 카메라 추적과 슬로우 모션이 작동합니다.');
  };
  ...
  return <main className="lunch-roulette lunch-roulette--original"><header className="lunch-roulette__hero"><span>MARBLE ROULETTE · ORIGINAL ENGINE</span><h1>점심 내기 룰렛</h1><p>원본 마블 룰렛의 물리·카메라·결승 연출에 팀 참가자 설정을 연결했습니다.</p></header>
    <section className="original-game"><div className="original-game__stage"><div className="original-game__hud"><span><i /> LIVE MARBLE RACE</span><b>{isRacing ? 'RACE IN PROGRESS' : winnerMode === 'last' ? 'LAST PLACE MODE' : 'READY'}</b><small>{totalBalls} MARBLES</small></div><div className="original-game__canvas" ref={gameHostRef} /></div><aside className="physics-result"><span className="lunch-roulette__panel-label">{winnerMode === 'last' ? 'LAST PLACE PICK' : 'TODAY\'S PICK'}</span><h2>{winnerMode === 'last' ? '오늘의 꼴찌' : '오늘의 당첨자'}</h2>{results.length ? <ol>{results.map((result) => <li key={`${result.name}-${result.rank}`}><em>{winnerMode === 'last' ? '꼴찌' : `${result.rank}등`}</em><strong>{result.name}</strong><small>{winnerMode === 'last' ? 'LAST MARBLE SURVIVOR' : '결승선 통과'}</small></li>)}</ol> : <div className="physics-result__empty"><span>🎱</span><p>{winnerMode === 'last' ? <>끝까지 남은 마지막 마블의<br />주인이 꼴찌!</> : <>긴 코스를 끝까지 통과한<br />마블의 주인이 당첨!</>}</p></div>}<button type="button" onClick={() => void copyResult()} disabled={!results.length}>결과 복사</button></aside></section>
    <section className="participant-picker"><div className="participant-picker__head"><div><span>TEAM SCHEDULER</span><h2>참가자와 공 개수</h2><p>공유 스케줄 · 업무 트래커에 사용된 이름을 불러왔습니다.</p></div><b>{participants.length}명 참여 · 공 {totalBalls}개</b></div><div className="roulette-map-select"><span>맵 선택</span><div>{maps.map((map) => <button type="button" key={map.index} className={map.index === mapIndex ? 'active' : ''} disabled={isRacing || !isEngineReady} onClick={() => changeMap(map.index)}>{map.title}</button>)}</div></div>
      <div className="participant-table"><div className="participant-table__label"><span>참여</span><span>이름</span><span>공 개수</span></div>{isLoadingNames ? <p className="participant-loading">팀원 이름을 불러오는 중...</p> : teamNames.map((name) => <div className="participant-row" key={name}><label><input type="checkbox" checked={selected[name]?.included ?? false} onChange={(event) => updateMember(name, { included: event.target.checked })} disabled={isRacing} /><span /></label><strong>{name}</strong><div className="ball-stepper"><button type="button" onClick={() => updateMember(name, { balls: Math.max(1, (selected[name]?.balls || 1) - 1) })} disabled={isRacing}>−</button><b>{selected[name]?.balls || 1}개</b><button type="button" onClick={() => updateMember(name, { balls: Math.min(5, (selected[name]?.balls || 1) + 1) })} disabled={isRacing}>+</button></div></div>)}</div><div className="manual-member"><input value={manualName} disabled={isRacing} onChange={(event) => setManualName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') addManualMember(); }} placeholder="목록에 없는 팀원 이름" /><button type="button" disabled={isRacing} onClick={addManualMember}>이름 추가</button></div><div className="physics-actions"><div className="winner-mode"><span>추첨 기준</span><button type="button" className={winnerMode === 'first' ? 'active' : ''} disabled={isRacing} onClick={() => setWinnerMode('first')}>1등</button><button type="button" className={winnerMode === 'last' ? 'active' : ''} disabled={isRacing} onClick={() => setWinnerMode('last')}>꼴찌</button></div>{winnerMode === 'first' && <div className="winner-count"><span>당첨 인원</span>{[1, 2, 3].map((count) => <button type="button" key={count} className={winnerCount === count ? 'active' : ''} disabled={count > participants.length || isRacing} onClick={() => setWinnerCount(count)}>{count}명</button>)}</div>}<button className="physics-start" type="button" onClick={startRace} disabled={participants.length < 2 || isRacing || !isEngineReady}>{isRacing ? '레이스 진행 중...' : winnerMode === 'last' ? '▶ 꼴찌 뽑기' : '▶ 레이스 시작'}</button></div><p className="physics-notice" role="status">{notice}</p><p className="roulette-license">Marble Roulette 원본 엔진 일부 사용 · Copyright © 2023 LazyGyu · MIT License</p></section>
  </main>;
};
