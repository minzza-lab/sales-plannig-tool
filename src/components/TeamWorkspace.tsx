...
import { CalendarDays, Check, ChevronLeft, ChevronRight, Download, FileUp, ListTodo, MessageCircle, Palmtree, Plus, Trash2, Users } from 'lucide-react'
...
export default function TeamWorkspace() {
  ...
  const removeEvent = async () => {
    if (!selectedEvent || !window.confirm(`「${selectedEvent.title}」 일정을 삭제할까요?`)) return
    setIsDeletingEvent(true)
    const { error } = await supabase.from('team_calendar_events').delete().eq('id', selectedEvent.id)
    setIsDeletingEvent(false)
    if (error) setNotice(`일정 삭제에 실패했습니다: ${error.message}`)
    else { setSelectedEvent(null); setNotice('공유 캘린더에서 일정을 삭제했습니다.'); await load() }
  }
  const exportMonthlySchedule = async () => {
    const XLSX = await import('xlsx')
    const year = month.getFullYear(); const monthNumber = month.getMonth() + 1; const lastDay = new Date(year, monthNumber, 0).getDate()
    const weekdays = ['일', '월', '화', '수', '목', '금', '토']
    const monthEvents = events.filter(item => { const date = new Date(item.start_at); return date.getFullYear() === year && date.getMonth() === month.getMonth() })
    const names = [...new Set(monthEvents.flatMap(item => item.assignee_names?.length ? item.assignee_names : [item.created_by_name]).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'ko-KR'))
    const codesByName = new Map(names.map(name => [name, new Map<number, string[]>()]))
    monthEvents.forEach(item => {
      const day = new Date(item.start_at).getDate(); const code = item.title.startsWith('[휴무 · ') ? item.title.match(/^\[휴무 · ([^\]]+)\]/)?.[1] : item.title
      const assignees = item.assignee_names?.length ? item.assignee_names : [item.created_by_name]
      assignees.forEach(name => { const person = codesByName.get(name); if (person && code) person.set(day, [...(person.get(day) ?? []), code]) })
    })
    const dates = Array.from({ length: lastDay }, (_, index) => index + 1)
    const rows: (string | number)[][] = [
      [`근무편성표(영업기획팀)-${year}년 ${String(monthNumber).padStart(2, '0')}월`], [],
      ['파트', '직위', '성 명', ...dates, '비고'],
      ['', '', '', ...dates.map(day => weekdays[new Date(year, monthNumber - 1, day).getDay()]), ''],
      ...names.map(name => ['영업기획팀', '', name, ...dates.map(day => (codesByName.get(name)?.get(day) ?? []).join(' / ')), '']),
    ]
    const sheet = XLSX.utils.aoa_to_sheet(rows)
    sheet['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: lastDay + 3 } }]
    sheet['!cols'] = [{ wch: 15 }, { wch: 10 }, { wch: 12 }, ...dates.map(() => ({ wch: 7 })), { wch: 20 }]
    sheet['!rows'] = [{ hpt: 26 }]
    const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, sheet, '스케쥴(최종)')
    XLSX.writeFile(workbook, `영업기획팀 스케줄 ${year}년 ${String(monthNumber).padStart(2, '0')}월.xlsx`)
    setNotice(`${year}년 ${monthNumber}월 캘린더 일정을 엑셀 양식으로 내보냈습니다.`)
  }
  ...
  return <div className="team-workspace animate-fade-in">
    ...
    {selectedEvent && <div className="workspace-modal-backdrop" onMouseDown={() => setSelectedEvent(null)}><section className="workspace-modal event-detail" onMouseDown={e => e.stopPropagation()}><header><div><p className="workspace-eyebrow">공유 스케줄</p><h2>{selectedEvent.title}</h2></div><button onClick={() => setSelectedEvent(null)}>×</button></header><p className="detail-description">{selectedEvent.description || '등록된 상세 내용이 없습니다.'}</p><div className="detail-meta"><span>{selectedEvent.all_day ? '종일 일정' : `${displayDate(selectedEvent.start_at)} ${new Intl.DateTimeFormat('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(selectedEvent.start_at))}`}</span><span>종료 {displayDate(selectedEvent.end_at)}</span><span>담당 {selectedEvent.assignee_names?.join(', ') || '미정'}</span><span>등록 {selectedEvent.created_by_name || '사용자'}</span></div><button className="workspace-danger" onClick={() => void removeEvent()} disabled={isDeletingEvent}><Trash2 size={16} /> {isDeletingEvent ? '삭제 중...' : '이 일정 삭제'}</button></section></div>}
