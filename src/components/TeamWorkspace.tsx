...
import { CalendarDays, Check, ChevronLeft, ChevronRight, ListTodo, MessageCircle, Plus, Users } from 'lucide-react'
import { supabase } from '../lib/supabase'
import SalesScheduler from './SalesScheduler'
import './TeamWorkspace.css'

type View = 'calendar' | 'sales' | 'tracker'
...
export default function TeamWorkspace() {
  ...
  const [composer, setComposer] = useState<'event' | 'task' | null>(null)
  ...
  const saveEvent = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await supabase.from('team_calendar_events').insert({
      title: String(form.get('title')).trim(), description: String(form.get('description')).trim() || null, start_at: start,
      end_at: end, all_day: form.get('all_day') === 'on', color: String(form.get('color')), assignee_names: String(form.get('assignees')).split(',').map(v => v.trim()).filter(Boolean),
      created_by: owner.id, created_by_name: owner.name, department: owner.department,
    })
    setIsSaving(false); if (error) setNotice(`일정 저장에 실패했습니다: ${error.message}`); else { setComposer(null); setNotice('공유 일정이 저장되었습니다.'); await load() }
  }
  ...
  return <div className="team-workspace animate-fade-in">
    <header className="team-workspace-header"><div><p className="workspace-eyebrow"><Users size={15} /> TEAM WORKSPACE</p><h1>공유 스케줄러 · 업무 트래커</h1><p>팀 일정과 업무 진행 상황을 한곳에서 함께 관리합니다.</p></div>{view !== 'sales' && <button className="workspace-primary" onClick={() => setComposer(view === 'calendar' ? 'event' : 'task')}><Plus size={18} /> {view === 'calendar' ? '일정 등록' : '업무 등록'}</button>}</header>
    <div className="workspace-tabs"><button className={view === 'calendar' ? 'active' : ''} onClick={() => setView('calendar')}><CalendarDays size={17} />공유 스케줄</button><button className={view === 'sales' ? 'active' : ''} onClick={() => setView('sales')}><span>₩</span>판매 스케줄 · 실적</button><button className={view === 'tracker' ? 'active' : ''} onClick={() => setView('tracker')}><ListTodo size={17} />업무 트래커 <span>{tasks.filter(t => t.status !== 'done').length}</span></button></div>
    {notice && <div className="workspace-notice">{notice}<button onClick={() => setNotice('')}>×</button></div>}
    {view === 'calendar' ? <section className="calendar-panel"><div className="calendar-toolbar"><button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}><ChevronLeft size={19} /></button><strong>{month.getFullYear()}년 {month.getMonth() + 1}월</strong><button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}><ChevronRight size={19} /></button><button className="today-button" onClick={() => setMonth(new Date(new Date().getFullYear(), new Date().getMonth(), 1))}>오늘</button></div><div className="calendar-grid calendar-weekdays">{['일','월','화','수','목','금','토'].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-grid calendar-days">{days.map(day => { const key = dateKey(day); const today = key === dateKey(new Date()); return <div className={`calendar-day ${day.getMonth() !== month.getMonth() ? 'other-month' : ''} ${today ? 'today' : ''}`} key={key}><time>{day.getDate()}</time>{(eventByDay[key] ?? []).slice(0, 3).map(item => <button key={item.id} title={item.description} className={`calendar-event ${item.color}`}><span>{item.all_day ? '종일' : new Intl.DateTimeFormat('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(item.start_at))}</span>{item.title}</button>)}{(eventByDay[key]?.length ?? 0) > 3 && <small>+{eventByDay[key].length - 3}개 일정</small>}</div> })}</div></section> : view === 'sales' ? <SalesScheduler /> : <section className="tracker-board">{(['todo', 'in_progress', 'done'] as TaskStatus[]).map(status => <div className="task-column" key={status}><header><h2>{statusLabels[status]}</h2><span>{tasks.filter(task => task.status === status).length}</span></header><div className="task-list">{tasks.filter(task => task.status === status).map(task => <article className="task-card" key={task.id} onClick={() => void openTask(task)}><div className="task-card-top"><span className={`priority ${task.priority}`}>{task.priority === 'high' ? '높음' : task.priority === 'low' ? '낮음' : '보통'}</span>{task.status !== 'done' && <button title="다음 상태로" onClick={e => { e.stopPropagation(); void moveTask(task, status === 'todo' ? 'in_progress' : 'done') }}><Check size={15} /></button>}</div><h3>{task.title}</h3>{task.description && <p>{task.description}</p>}<footer>{task.due_date ? <span className={new Date(`${task.due_date}T23:59:59`) < new Date() ? 'overdue' : ''}>마감 {displayDate(task.due_date)}</span> : <span>마감일 없음</span>}{task.assignee_names?.length > 0 && <span>👤 {task.assignee_names.join(', ')}</span>}</footer></article>)}{tasks.filter(task => task.status === status).length === 0 && <p className="empty-column">등록된 업무가 없습니다.</p>}</div></div>)}</section>}
    {composer && <div className="workspace-modal-backdrop" onMouseDown={() => setComposer(null)}><form className="workspace-modal" onSubmit={composer === 'event' ? saveEvent : saveTask} onMouseDown={e => e.stopPropagation()}><header><h2>{composer === 'event' ? '공유 일정 등록' : '업무 등록'}</h2><button type="button" onClick={() => setComposer(null)}>×</button></header><label>제목<input name="title" required placeholder={composer === 'event' ? '예: 9월 판촉 회의' : '예: 추석 패키지 안내문 검토'} autoFocus /></label>{composer === 'event' ? <><div className="two-fields"><label>시작<input name="start_at" type="datetime-local" defaultValue={localDateTime(new Date())} required /></label><label>종료<input name="end_at" type="datetime-local" defaultValue={localDateTime(new Date(Date.now() + 3600000))} required /></label></div><label className="check-label"><input name="all_day" type="checkbox" /> 종일 일정</label><label>색상<select name="color">{colors.map(color => <option key={color} value={color}>{color === 'blue' ? '파랑' : color === 'purple' ? '보라' : color === 'orange' ? '주황' : '초록'}</option>)}</select></label></> : <><div className="two-fields"><label>마감일<input name="due_date" type="date" /></label><label>우선순위<select name="priority"><option value="medium">보통</option><option value="high">높음</option><option value="low">낮음</option></select></label></div></>}<label>담당자 <small>쉼표로 구분</small><input name="assignees" placeholder="예: 김영업, 이기획" /></label><label>내용<textarea name="description" rows={4} placeholder="팀원이 알아야 할 업무 내용과 참고사항을 적어주세요." /></label><button className="workspace-primary" disabled={isSaving}>{isSaving ? '저장 중...' : '팀에 공유하기'}</button></form></div>}
