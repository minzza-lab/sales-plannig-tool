...
export default function WaterOperationsDashboard() {
  ...
  return <div className="water-hub">
    ...
    <section className="water-hub-grid">
      ...
      <article className="water-hub-card rental-status"><header><div><span>RENTAL UTILIZATION</span><h2>대여상품 가동 현황</h2></div><Link to="/tools/water-operations-analysis">대여 상세 →</Link></header><div className="rental-status-list">{rentals.length ? <>{rentals.filter((item) => item.fixed).map((item) => { const rate = item.capacity ? item.used / item.capacity * 100 : 0; return <div key={item.name}><div><b>{item.name}</b><span>{item.used.toLocaleString()}개 · {compactWon(item.amount)}</span></div><strong>{rate.toFixed(1)}%</strong><i><em style={{ width: `${Math.min(100, rate)}%` }}/></i><small>보유 {item.capacity.toLocaleString()}개 중</small></div> })}{rentals.some((item) => !item.fixed) && <b style={{display: 'block', margin: '14px 0 4px', fontSize: '12px', color: '#4d756d'}}>기타 대여상품</b>}{rentals.filter((item) => !item.fixed).map((item) => { const rate = item.capacity ? item.used / item.capacity * 100 : 0; return <div key={item.name}><div><b>{item.name}</b><span>{item.used.toLocaleString()}개 · {compactWon(item.amount)}</span></div><strong>{rate.toFixed(1)}%</strong><i><em style={{ width: `${Math.min(100, rate)}%` }}/></i><small>당일 발권객 {item.capacity.toLocaleString()}명 대비</small></div> })}</> : <p className="no-detail">이 일자의 대여 상세 수집 데이터가 없습니다.</p>}</div></article>
