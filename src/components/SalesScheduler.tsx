...
export default function SalesScheduler() {
  ...
  return <section className="sales-scheduler">
    ...
    <section className="sales-product-list"><h2>등록 상품 <span>{filteredProducts.length}</span></h2>{filteredProducts.length === 0 ? <p>선택한 분류에 등록된 판매 상품이 없습니다.</p> : <div>{filteredProducts.map(product => <button key={product.id} onClick={() => { setSelected(product); setModal('detail') }}><i className={colorFor(product)} /><span><strong>{product.title}</strong><em>{labels[product.category]} · {product.company_name} · 판매 {product.sales_start} ~ {product.sales_end}</em></span><b>순매출 {money(Number(product.net_amount))}원<small>수량 {money(Number(product.net_quantity))}건 · 목표 {money(Number(product.target_amount))}원</small></b></button>)}</div>}</section>
