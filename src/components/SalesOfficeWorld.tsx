...
import type { InvestigationPlan, MeetingReport, ProposalRequest } from './SalesOperationOffice'
import './office-engine/office-world.css'
import './SalesOfficeWorld.css'
import './SalesOfficeProposal.css'

export default function SalesOfficeWorld({ syncState, onAgentAction, onPlanInvestigation, onAskProposal }: { syncState: 'idle' | 'running' | 'completed' | 'failed'; onAgentAction: (department: string) => void; onPlanInvestigation: (instruction: string, requestedDepartment: string) => InvestigationPlan; onAskProposal: (request: ProposalRequest) => Promise<MeetingReport> }) {
  ...
  const [proposal, setProposal] = useState<MeetingReport | null>(null)
