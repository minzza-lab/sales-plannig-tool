...
export default function WaterOperationsAnalysis() {
  ...
  return <div className="water-ops-page">
    ...
    <section className="water-ops-card">
      ...
      {active?.rental.length ? <><div className="rental-products">{rentals.filter((item) => FIXED_CAPACITY[item.name]).map(renderRentalCard)}</div>{rentals.some((item) => !FIXED_CAPACITY[item.name]) && <section style={{marginTop: '18px'}}><div className="water-ops-heading"><div><Waves size={16}/><div><span>OTHER RENTALS</span><h2>기타 대여상품</h2></div></div><small>당일 발권객 {Math.max(0, used(active.ticket)).toLocaleString()}명 · 상품별 매출 · 발권객 대비 사용률</small></div><div style={{display: 'grid', gap: '6px', marginTop: '12px'}}>{rentals.filter((item) => !FIXED_CAPACITY[item.name]).map(renderOtherRentalRow)}</div></section>}</> : <Empty text="선택 날짜의 대여상품 상세가 없습니다." />}
    </section>
  </div>
}
